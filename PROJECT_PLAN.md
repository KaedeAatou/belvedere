...
| # | タスク | 内容 | 重要度 |
|---|---|---|---|
| U-Auth1 | **Firebase Console で Authentication を有効化** | Firebase Console → Project (belvedere-dev-atrium) → Authentication → Get Started → Google provider 有効化、許可 email に `owner@example.com` 追加 | 🔥 |
...
| # | タスク | 内容 | 重要度 |
|---|---|---|---|
| U-Sub1 | **Proto Pedia アカウント作成** | proto.pedia.jp で `owner@example.com` 登録。提出 STEP③ で必須 | 🟧 |
