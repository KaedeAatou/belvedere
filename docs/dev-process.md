...
## モデル運用 (コスト最適化 / 2026-06-10 制定)

| 役割 | モデル | 使う場面 |
|---|---|---|
| 計画・設計・レビューゲート | **Fable** (高コスト、必要時のみ) | リファクタ計画策定 / フェーズ完了レビュー / アーキ岐路の判断 / 「計画を見直して」系の依頼 |
| 実装・テスト・修正 | **Opus / Sonnet** (日常) | 計画ドキュメントに従った実装 / bug fix / test 追加 |
| 探索・一括検索 | Explore subagent (安価) | 影響範囲調査 / 命名規則探し |

運用ルール:
- Fable が書く計画は **実行モデルが追加判断なしで進められる粒度** にする
  (ファイルパス / 手順 / 検証ゲート / commit 境界 / エスカレーション基準を明記)。
- 実行モデルは計画の「不可侵リスト」「エスカレーション基準」に従い、逸脱が必要なら止まって確認。
- 現行の計画ドキュメント: `docs/refactoring-plan.md` (R1〜R9 フェーズ)。
...
3. e2e の workspace 戦略
   - Stage 1: mygolanglearn@gmail.com の ws-belvedere 共用 (allowlist 既存活用)
