...
| 2026-06-11 日中 | T8 e2e + backend bug 3 件 | ✅ 完了 / **CI 緑 11/11** | 2c7f639 / e65cace / eaecb29 / a96f3a7 | estimation.spec (ポーカー happy path + finding ピル) + backlog.spec に編集/削除 + RefinementPage/DetailSheetPage。**T8 が unit test で見えない backend 統合バグを 3 件捕捉**: ① 作成 API が type 未保存 (種別セレクタ無効化 + ポーカー導線断絶) ② CORS が PUT 未許可 → 投票ブロック ③ adopt が EstimationView 非返却 + getEstimation が adopted を 404 → 「採用済」表示不可。各々修正 + 回帰テスト追加。e2e 11/11 緑 |
| 2026-06-11 日中 | **UI epic 完了** | ✅ 全 7 フェーズ | — | T0→R3→T5→T9→T7→T10→T8 完走。typecheck 11/11 / test 156/156 / e2e 11/11 全緑。Firestore seed 投入で儀式が実 WC データ表示。全画面 §V 目視 + ポーカー/ピル/編集/削除を e2e で固定。残: e2e 汚染チケット掃除 (--clean、ユーザー承認待ち) |
