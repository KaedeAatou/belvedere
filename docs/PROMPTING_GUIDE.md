...
| Agent | 役割 | 重要原則 |
|---|---|---|
| Planner | Sprint Planning 支援 / Sprint Goal / 計画SP vs velocity (過剰計画) / チケット品質 (DoD/SP/US紐付け) 診断 | 議題ドラフトはチケット品質診断を済ませてから作る |
...
<reasoning>
  以下の順番で考えてください:
  1. sprint.get で現スプリントの velocity 実績と Sprint Goal を確認
  ...
  5. 議題ドラフトを生成 (品質要修正リスト + 計画SP vs velocity 比較 + Epic 進捗)
</reasoning>
```
