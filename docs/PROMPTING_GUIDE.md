...
### 例の作り方
- seed (`packages/seed/`) の WC-101..112 / wind-001..008 から拾う (ID整合保証)
...
```xml
<rules>
  <rule>根拠IDは必ず WC-xxx / wind-xxx / slack:xxx / gh:xxx 形式で引用すること</rule>
...
```xml
<dont>
  <item>seed の WC-101..112 / wind-001..008 を編集してはいけない</item>
