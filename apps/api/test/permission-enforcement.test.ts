...
const DRIVERS: Record<Action, (repo: RepoContainer, ctx: HandlerContext) => Promise<HandlerResult<unknown>>> = {
  ...
  // ticket.write / agent.invoke は「全ロール許可」。ticket CRUD はハンドラ非ゲート (足場)、
  // agent.invoke は app.ts 内 (Hono inline) で全ロール許可。両者とも handler 単体ドライバを持たないため
  // ここでは検証対象外 (can() レベルは permissions.test.ts が全組合せを固めている)。
  'ticket.write': async () => ({ ok: true, status: 200, body: null }),
  'agent.invoke': async () => ({ ok: true, status: 200, body: null }),
};

// 実際にゲートが handler に配線されている action だけを回す (ticket.write/agent.invoke は上記理由で除外)。
