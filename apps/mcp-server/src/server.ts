...
export interface McpClientConfig {
  ...
  baseUrl: string;
  /**
   * API に送る Bearer トークン。次のいずれか:
   *   - per-user API キー (`blv_...` / Web 設定画面で本人が発行 / 本人として通る)
   *   - MCP サービストークン (API の MCP_SERVICE_TOKEN と一致 / 機械用 svc:mcp)
   */
  token: string;
...
export function createBelvedereMcp(config: McpClientConfig): BelvedereMcp {
  ...
  async function callTool(
    ...
    if (!config.token) {
      return errorResult(
        'BELVEDERE_MCP_TOKEN が未設定です (per-user API キー blv_... または サービストークン)。' +
