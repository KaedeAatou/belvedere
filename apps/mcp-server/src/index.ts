...
//
// 認証は BELVEDERE_MCP_TOKEN に Bearer を 1 本渡す (env):
//   - per-user API キー (推奨・本人用): Web 設定画面で発行した `blv_...`。本人として動き、
//     所属する全ワークスペースにアクセスできる。
//   - サービストークン (機械用 / 提出デモ): API の MCP_SERVICE_TOKEN と一致する値。
...
//   claude mcp add belvedere --env BELVEDERE_MCP_TOKEN=<blv_... または サービストークン> \
...
async function main(): Promise<void> {
  ...
  // token は per-user API キー (blv_...) か サービストークンのどちらか (API 側が prefix で判別)。
  const authMode = token ? (token.startsWith('blv_') ? 'api-key' : 'service token') : 'NONE';
