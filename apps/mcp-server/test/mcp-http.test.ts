...
describe('MCP HTTP ↔ API: 認証ガード', () => {
  // 認証は BELVEDERE_MCP_TOKEN (per-user API キー blv_... または サービストークン) を 1 本渡す。
  // refresh token モードは attack surface 削減のため廃止 (2026-06-17 / ①+③ に集約)。
  it('token 未設定 → HTTP を投げる前にガードエラー (BELVEDERE_MCP_TOKEN を案内)', async () => {
    const mcp = createBelvedereMcp({
      baseUrl: 'https://x',
      token: '',
      workspaceId: 'ws-belvedere',
  ...
  it('token 設定済み → その値をそのまま Bearer に載せて API を叩く', async () => {
    const auths: (string | null)[] = [];
    const mcp = createBelvedereMcp({
      baseUrl: 'https://api.example',
      token: 'blv_user-key-xyz',
      workspaceId: 'ws-belvedere',
      fetch: (req) => {
    ...
    const res = read(await mcp.callTool('belvedere_ticket_list', {}));
    expect(res.isError).toBe(false);
    expect(auths).toEqual(['Bearer blv_user-key-xyz']);
  });
});
