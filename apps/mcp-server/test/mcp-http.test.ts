...
describe('MCP HTTP ↔ API: 認証 (service token)', () => {
  ...
  it('誤ったサービストークン → API 401 (Firebase 検証に落ちて invalid_token)', async () => {
    const mcp = makeMcp(repo, { token: 'wrong-token' });
    const res = read(await mcp.callTool('belvedere_ticket_list', {}));
    expect(res.isError).toBe(true);
    expect(res.text).toContain('401');
  });

  it('トークン未設定 → HTTP を投げる前にガードエラー', async () => {
  ...
  it('env MCP_SERVICE_TOKEN 未設定なら正しいトークンでも素通りしない (= 認証パス無効)', async () => {
    delete process.env.MCP_SERVICE_TOKEN;
    const mcp = makeMcp(repo, { token: TOKEN });
    const res = read(await mcp.callTool('belvedere_ticket_list', {}));
    // env 未設定 → matchesServiceToken は常に false → Firebase 検証に回って 401
    expect(res.isError).toBe(true);
    expect(res.text).toContain('401');
  });
});
