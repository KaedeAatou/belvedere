...
describe('MCP HTTP ↔ API: 本人認証 (refresh token モード)', () => {
  // 実 Firebase 検証 (verifyIdToken) はモックできないため、MCP クライアントの「refresh token →
  // securetoken で ID token 交換 → Bearer 付与 → キャッシュ」のロジックだけを mock fetch で隔離検証する。
  function fakeFetch(opts: { onApi?: (auth: string | null) => void } = {}): { fetch: (req: Request) => Response; exchanges: () => number } {
    let exchanges = 0;
    const f = (req: Request): Response => {
      const url = new URL(req.url);
      if (url.host === 'securetoken.googleapis.com') {
        exchanges++;
        return new Response(JSON.stringify({ id_token: 'fake-id-token-xyz', expires_in: '3600' }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      opts.onApi?.(req.headers.get('Authorization'));
      return new Response(JSON.stringify([]), { status: 200, headers: { 'Content-Type': 'application/json' } });
    };
    return { fetch: f, exchanges: () => exchanges };
  }

  it('refresh token を securetoken で交換し ID token を Bearer に載せる / 期限内はキャッシュして再交換しない', async () => {
    const auths: (string | null)[] = [];
    const ff = fakeFetch({ onApi: (a) => auths.push(a) });
    const mcp = createBelvedereMcp({
    ...
    const r1 = read(await mcp.callTool('belvedere_ticket_list', {}));
    const r2 = read(await mcp.callTool('belvedere_ticket_list', {}));
    expect(r1.isError).toBe(false);
    expect(r2.isError).toBe(false);
    expect(ff.exchanges()).toBe(1); // 2 回 API を叩いても交換は 1 回 (キャッシュ)
    expect(auths.length).toBe(2);
    expect(auths.every((a) => a === 'Bearer fake-id-token-xyz')).toBe(true);
  });

  it('refresh / service token 両方未設定 → ガードエラー', async () => {
    const mcp = createBelvedereMcp({
      baseUrl: 'https://x',
      token: '',
      workspaceId: 'ws-belvedere',
    ...
    const res = read(await mcp.callTool('belvedere_ticket_list', {}));
    expect(res.isError).toBe(true);
    expect(res.text).toContain('BELVEDERE_REFRESH_TOKEN');
  });

  it('refresh token モードで firebaseApiKey 未設定 → 明確なエラー', async () => {
    const mcp = createBelvedereMcp({
      baseUrl: 'https://x',
      token: '',
      refreshToken: 'rt',
    ...
    const res = read(await mcp.callTool('belvedere_ticket_list', {}));
    expect(res.isError).toBe(true);
    expect(res.text).toContain('firebaseApiKey');
  });
});
