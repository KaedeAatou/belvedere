...
export E2E_ROBOT_UID='SUJvQ0XsrNQRhodNpM3BhAAKRn62'  # mygolanglearn@gmail.com の Firebase UID
...
| 症状 | 対処 |
|---|---|
| `Firebase signInWithCustomToken failed: auth/internal-error` | E2E_ROBOT_UID が Firebase Auth に存在しない UID。mygolanglearn@gmail.com で 1 回ログインしてから Firebase Console で UID 確認 |
...
| `failed to create ticket: 403 invitation_required` | robot UID が emailAllowlist で bootstrap されない。Stage 1 では mygolanglearn の UID を robot として使う想定なので、その UID で必ず /login → ログイン経験させてから e2e 実行 |
