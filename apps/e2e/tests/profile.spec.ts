...
test('settings/profile で email/role/workspace 表示 + /api/me で admin/owner role 確認', async ({ authedPage, apiBaseUrl }) => {
  ...
  // 権限再設計 (2026-06-23) で robot は admin に。本番 Firestore の migration は提出後のため、
  // 移行期は永続 role が owner(旧) のまま出る場合もある → owner|admin の両方を許容する。
  await expect(profile.roleBadge).toContainText(/owner|admin/i);
  ...
  expect(result.status, `body=${result.body}`).toBe(200);
  expect(result.body).toMatch(/"(owner|admin)"/);
});

test('右上 UserMenu ドロップダウンに admin/owner バッジ + email 表示', async ({ authedPage }) => {
  await authedPage.goto('/', { waitUntil: 'networkidle' });

  await authedPage.locator('.avatar-btn').click();
  await expect(authedPage.locator('.dropdown')).toBeVisible({ timeout: 5_000 });
  await expect(authedPage.locator('.badge-role')).toContainText(/owner|admin/i);
