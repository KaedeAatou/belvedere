...
| 4 | 0:55-1:20 | ピル (DoDなし/SP未定) に 3 秒ズーム → AI パネル:「**WC-114 は親Epicの戦略意図(rationale)からドリフトしていない?判定して。**」→ 回答 | 「空欄のチェックは AI ではなく決定論のルールが瞬時に出します。if 文でできることを AI の仕事とは呼びません。AI の仕事はここから。— このチケットは、親 Epic の戦略意図からドリフトしている。Epic の Why と成功指標を読んだ上で、理由まで答えます」 |
