...
### 必須メタ
- **作品ステータス**: 完成 (ハッカソン提出)
- **タイトル**: Belvedere — スクラムの運営はAIに、人間は価値の判断に。
- **タグ**: `findy_hackathon` を必ず含める (+ `Gemini` `CloudRun` `ADK` `Elasticsearch` `Scrum` 等)
- **動画 (YouTube/Vimeo URL)**: 撮り直し後に差し替え (現行: https://youtu.be/w-eYgJbDRt0 / 台本は本ファイル §4 + シーン差し替え案)
- **システムアーキテクチャ図**: `docs/submission-diagram.md` の Eraser DSL → PNG

> タイトル/概要の注意: **Belvedere 自体はコードを書く AI ではない** (「AI が開発する」は時代文脈としてストーリーに置き、Belvedere の受け持ちは運営と価値判断の支援だと誤解なく書く)。ProtoPedia は**概要の文頭が作品一覧に表示される**ため、最初の一文で差別化を言い切る。

### 概要 (≤100 字 / 文頭が一覧に出る)
スクラムの運営はAIに、人間は価値の判断に。儀式ごとのAIスクラムマスターが前回のTry遵守を実数値で検証し、ゴールからチケットへの価値のつながりを意味判断するJira型PMサービス。

### ストーリー① 課題と背景
AI が開発を担い始めた時代、人間に残る最も重要な仕事は「**何を作るとビジネス価値があるか**」の判断です。ところが現実のスクラムは、儀式 (イベント) もイベントごとのルールも多く、回すこと自体が仕事になる ── DoD 空欄・SP 未定のチケットの手入れ、ふりかえりで決めた Try の追跡、Epic の奥に埋もれた Why の掘り起こし。運営コストを人間 (特にスクラムマスター) が払い続けた結果、価値を考える時間が消え、**チケットは価値のつながりを失ってただの ToDo リストに退化する**。Belvedere はこの運営コストを AI スクラムマスターに肩代わりさせ、人間を価値の判断に引き戻す PM サービスです (コードを書く AI ではなく、スクラム運営を担う AI です)。

### ストーリー② 利用ユーザー
スクラムで開発する PO / SM / Dev / EM。SM はイベントごとに違うルールの運営と決めごと追跡の疲弊を、PO は価値と優先順位の説明コストを、Dev は「このチケットは何のためか」を見失う迷子を、EM は「うちのスクラムは健康か」の可視化を AI に肩代わりさせたい。

### ストーリー③ 特徴 ── つくる・まわす・とどける
スクラムはイベントごとにルールが違い、運営がいちばん重い。だから Belvedere は **儀式 (イベント) ごとに専用画面を持ち、それぞれに専門の AI エージェントが常駐** する (Planning / Daily / Refinement / Review / Retrospective の 5 画面 + 単一窓口の Orchestrator = AI スクラムマスター)。Jira の 1 ボードとの最大の違いはここ。

その上で、AI の仕事は「空欄チェック」ではない。3 本柱は全て実機 (実 Gemini + 実データ) で動作を実証済み:
...
- **(c) 二層設計** ── DoD 空欄・SP 未定・親 Story なし等の **値のチェックで済む欠落は、AI ではなくルールエンジン (17 観点) が決定論で即時ピル表示** する。AI の推論は (a)(b) のような **文脈とチケットの中身を読む意味判断だけに投資** する。「空欄検出を AI がやりました」とは言わない ── それは if 文でできる。AI にしかできない判断に絞ることが、この作品の AI エージェントとしての主張。
