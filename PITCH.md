...
1. 反復をまたぐ記憶: ふりかえりで決めた Try をベクトル検索(Gemini Embeddings)で呼び出し、「前回決めたことは今のスプリントで守られているか」を実際の数値で検証します。実例として、Try「velocity を超えて計画しない」に対し、計画 SP 合計 68 と velocity 実績 27 という具体的な数値から違反を毎回正しく検出します。人間なら議事録を掘り返す作業が、AI への質問 1 つで返ってきます。
