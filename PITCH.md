...
2. ビジネス直結の意味判断: プロダクトゴール → スプリントゴール → Epic の戦略意図(rationale) → チケット という価値の連鎖を、字面の一致ではなく意味のレベルで判定します。「このチケットはゴールに貢献しているか」「親 Epic の戦略意図から逸脱(ドリフト)していないか」を AI が判断し、ドリフトしているチケットを名指しで指摘します。
...
まわす: ふりかえりで決まった Try は次の儀式の判断材料として自動的に積み上がり、使うほどチームに最適化されていきます。プロンプトの改善は自動評価(agent eval)を CI に組み込んで品質の後退を防いでいます。鍵を使わない CI/CD(GitHub Actions から Cloud Build 経由で Cloud Run へ)を構築し、テストを通過した特定のコミットだけを本番へ昇格させる仕組みも実装しています。さらに Model Context Protocol(MCP)経由で開発ツール(Claude Code)から Belvedere 自身を操作できるようにし、実際に別プロジェクトのスクラム運営を Belvedere で 6 スプリント分回して、36 件の不具合・改善点を検出し 34 件を修正するという自己改善のループを実践しました。
