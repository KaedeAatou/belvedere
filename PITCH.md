...
## 4a. 提出動画 台本 確定版 (2026-07-11 / 約 2:10 / prod 実機)

> 全シーン **prod (belvedere-scrum.web.app / demo アカウント / ws-belvedere)**。シーン単位クリップ録画 → Gemini 待ちは編集カット → ナレーション後録り。テロップ 2 枚のみ (Scene 3「計画 68SP > velocity 実績 27SP」/ Scene 7 技術+URL)。空応答 (「ステータス: succeeded」) が出たクリップは同じ質問を再送信して撮り直し。**3 質問とも prod 実機でリハーサル済み** (① 7/9 + 検算根治後 dev / ②′ 7/11 prod WC-114 / ③ 総合診断 dev)。
...
| 3 | 0:35-0:55 | AI パネル:「**前回のふりかえりで決めたTryは、今のスプリントで守られている?**」→ 即答 | 「前のスプリントで決めた"決めごと"、覚えていますか? AI は覚えています。— 違反しています。計画 68 ポイントは実績ベロシティ 27 を大幅に超過。議事録を掘り返す仕事が、質問 1 つで返ります」 |
| 4 | 0:55-1:20 | ピル (DoDなし/SP未定) に 3 秒ズーム → AI パネル:「**WC-114 は、親Epicの戦略意図(rationale)から外れていない?判定して。**」→ 回答 | 「空欄のチェックは AI ではなく決定論のルールが瞬時に出します。if 文でできることを AI の仕事とは呼びません。AI の仕事はここから。— このチケットは、親 Epic の『なぜやるか』から外れている。Epic の Why と成功指標を読んだ上で、理由まで答えます」 |
| 5 | 1:20-1:40 | AI パネル:「**バックログを総合診断して。過去のふりかえりで決めたTryとプロダクトゴールも踏まえて、何から直すべきか優先順位を付けて。**」→ agent.invoke チップ×2 + 統合回答 | 「複数の観点が要る質問では、Orchestrator が専門エージェントを協議に招集。品質診断・Try の遵守・ゴール整合、3 つの材料を 1 つの答えに統合します」 |
| 6 | 1:40-1:55 | アーキ図 (CI/CD レーン入り) | 「Cloud Run と Gemini、ADK。全テストと AI の自動評価をゲートに、検証済みのコミットだけが本番へ。ふりかえりの決めごとは次の判断材料に積み上がり、使うほどチームに賢くなります」 |
| 7 | 1:55-2:10 | タイトルカード | 「スクラムの運営は AI に、人間は価値の判断に。Belvedere。」 |
