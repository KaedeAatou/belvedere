...
3. **ユーザビリティ** → 採用UI v3/13 (Hand × Digital) でデモ
...
スライド: 採用UI v3/13 (Hand × Digital) の画面、Sprint Board

> 「**Belvedere** は、形骸化したスクラムを **AI が「チケット品質」と「儀式運営」の両面から底上げする** プロジェクト管理サービスです。」

> 「チケットを書くのは人間。AI Agent は **DoD・User Story紐付け・Story Point** の不足を補い、儀式の運営を引き受けます。」
...
6. **(15秒)** 健全性ダッシュボード → 5儀式スコア表示、「Daily が -8 で要注意」のアラート
...
- **Cloud Run**: 各エージェントを独立サービスに
- **Gemini API + ADK**: マルチエージェント (Planner/Daily/Reviewer/Retrospective/Orchestrator)
- **Firestore + Vector Search**: 過去ふりかえりからの学習
- **Cloud Build / Deploy**: OWASP リリースゲート付き
...
- [ ] スライドデザイン (採用UI v3/13 の世界観に合わせる: 手書き×デジタル)
- [ ] 90秒デモの録画 (Cloud Run preview revision)
- [ ] アーキ図 (Excalidraw / 手書き風 — 採用UIと整合)
- [ ] 質疑用バックアップスライド (LLMコスト / セキュリティ / 拡張性)
