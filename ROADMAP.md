...
- [ ] **R3: Demo/Live UI 統一** (DemoTicket→shared Ticket 18 ファイル / 4 画面の実 API 化。無人実行で保留、要・画面確認)
- [ ] **T5: 種別バッジ + 作成ダイアログ種別セレクタ + 行内 finding バッジ** (R3 後)
- [ ] **T7: 見積もりパネル (DetailSheet) + ポーリング** (R3 後)
- [ ] **T8: ポーカー happy-path + Backlog バッジ e2e** (R3 後)
