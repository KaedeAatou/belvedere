...
### Phase 1-B 認証 / 2026-06-10 〜 6/14 (5 日)

- [ ] **U-Auth1 (👤)**: Firebase Console で Authentication 有効化 + Google provider (15 分、docs/setup-firebase-auth.md 参照)
- [ ] `apps/api` に Firebase Admin SDK 認証ミドルウェア (Authorization: Bearer 検証)
- [ ] `apps/web` に Firebase JS SDK ログイン UI (`/login` ページ + signInWithPopup)
- [ ] Web → API リクエストに ID token 自動付与 (composable)
- [ ] **workspaceId 全層改修 (IDOR fix)**: RepoContainer / TicketQuery / 全 caller に workspaceId を通す
- [ ] `infra/firestore.rules` で個人 Google だけ read/write 許可

### Phase 1-C Web UI CRUD / 6/15 〜 6/19 (5 日)
