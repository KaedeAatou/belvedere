...
> 2026-06-25 改訂: **ADK を実体化 + RAG を GCP ネイティブ化**。Web 調査 (Anthropic「Building Effective Agents」/ 12-Factor Agents / Google ADK・A2A 公式) で **「自前 TS runAgent を本体(正本)に保ち、ADK は A2A 越しの専門ピアとして追加 (Strangler Fig)」** が best practice と確定 — 自前は不要にならない。(a) `apps/orchestrator-py` を実 ADK (google-adk 1.31 `LlmAgent`×6 + Refinement に 6観点取得 `FunctionTool`) で配線し、`to_a2a` で A2A 公開 (空き家の `NotImplementedError` 解消)。(b) **Refinement だけ** を `REFINEMENT_VIA_ADK=true` で A2A 越しに ADK ピアへ委譲 (コード固定ルート / **不達は TS runAgent へ自動 fallback = 退避路**)。本体の協議 (agent.invoke 深さ1) と他 4 儀式は TS のまま。(c) **RAG は差し込み式** (`KnowledgeSearcher` / `SEARCH_BACKEND`): 本番は GCP ネイティブ **Firestore Vector** (Gemini 埋め込み / 無料・無期限)、協賛 **Elastic** にも env 1 つで切替可能。(d) **MCP(垂直=ツール) と A2A(水平=エージェント間) は補完** で役割分担。全 flag 既定 OFF で本番ゼロ変更。AI 固有テスト (ID捏造防止 / RAG 引用 / 決定性 / prompt injection 耐性 / **A2A failover**) を追加。
