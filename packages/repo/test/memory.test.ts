...
describe('memory backend - listByUserId (workspace 解決 middleware 用)', () => {
  ...
  it('seed の owner (kagayayuuki) は 1 件の Workspace に所属', async () => {
    const memberships = await repo.members.listByUserId('kagayayuuki');
...
describe('memory backend - estimations (見積もりポーカー / T2)', () => {
  ...
  it('upsert → get / ticketId・status フィルタ / workspace 分離', async () => {
    await repo.estimations.upsert({
      ...
      ticketId: 'WC-101',
      status: 'voting',
      votes: [{ userId: 'kagayayuuki', value: 5, submittedAt: '2026-06-11T00:00:00Z' }],
