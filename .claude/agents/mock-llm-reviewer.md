...
廃止済 (2026-04-30): 翼メタファー (北翼/東翼/南翼/西翼) と「風 (WindEvent)」概念。Agent の役割は **チケット品質補助 (DoD/SP/User Story紐付け) + 5儀式運営補助 + 健全性可視化 + バックログリファインメント診断 (粒度/依存/valueImpact/priority×valueImpact ミスマッチ/SP分散/戦略整合性)** に絞られている。
...
- 各ロールが期待するツール名が `packages/tools/src/index.ts` の `buildTools()` に存在するか
  (`epic.list` / `ticket.list` / `ticket.quality.check` / `sprint.get` / `member.list` / `refinement.check` 等。出力は AI パネル提示で、自律 Slack 投稿系ツールは持たない)
...
- 自律性レベル (全 5 ロール L2 / L3・L4 不採用) と整合した語り口か (出力は AI パネル提示)

### 5. 動作確認

可能なら以下を実行して落ちないか確認:
- `pnpm typecheck`
- `pnpm --filter @belvedere/cli dev plan` / daily / refinement / review / retro 5種を順に実行
