...
**やりたい変更が複数あるときの判断 (2026-06-08 強化)**:
- **デフォルトは分割** (1 commit = 1 論理変更が大原則)。後で `git log --oneline` を見たとき、各 commit の意図が独立に読めることが審査基準 (B-5 実装力評価)
- 「実装」「設計ドキュメント更新」「skill / hook 更新」「リファクタ」が混在したら **必ず分割**
- 同じファイルに対する別目的の変更 (例: ROADMAP の「日付振り直し」と「新方針追加」) は、後でリビジョンを追える限り分割
- 分けると個々がビルドできない (テスト落ちる) なら 1 commit ── これ **だけ** が分割しない正当な理由
- 迷ったら分ける

> **削除済の例外条項 (2026-06-08)**: かつて「緊急で粒度を犠牲にしたい場合は『時間優先で 1 commit』と 3 行目に書いて例外として残す」という抜け道があったが、実運用で「時間優先」を理由に 5 ファイル 2 種別を 1 commit にまとめて怒られたため削除。**「時間が無い」は分割しない理由にしない**。3 commit を heredoc 3 回叩いても、1 commit に対する数十秒の差にすぎない。
...
| 時間が無いから 1 commit にまとめたい | **不可**。3 commit に分割。「時間優先」を理由に bundling しない (2026-06-08 強化) |
| `git add -A` / `git add .` | 禁止。ファイル名を明示指定 (.env / 秘密情報の巻き込み防止) |
