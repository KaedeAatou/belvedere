...
- `build_agents(use_real_adk: bool)` - `False` でスタブ dict / `True` で実 `LlmAgent`×6 (google-adk) を構築
...
- `belvedere-orchestrator` - pyproject の package name (TS workspace の `@belvedere/*` と内部識別子を揃える、再ブランド過渡期)
- `google-adk` / `google-genai` / `google-cloud-aiplatform` / `google-cloud-firestore` / `google-cloud-secret-manager` - GCP SDK 依存 (実装済 / `USE_REAL_ADK=true` で実 LlmAgent を構築)
